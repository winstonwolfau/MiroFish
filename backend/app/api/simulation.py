...
from ..services.graph_entity_reader import get_entity_reader
...
def get_graph_entities(graph_id: str):
    ...
    try:
        ...
        logger.warning(f"当前图后端能力未实现: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e)
        }), 501
    ...
    try:
        ...
        if run_state:
            ...
            if run_state.runner_status == RunnerStatus.COMPLETED:
                next_status = SimulationStatus.COMPLETED
            elif run_state.runner_status in [RunnerStatus.STOPPED, RunnerStatus.FAILED]:
                if state.status in [SimulationStatus.RUNNING, SimulationStatus.PAUSED]:
                    next_status = SimulationStatus.STOPPED

            if next_status and state.status != next_status:
...
@simulation_bp.route('/<simulation_id>/profiles', methods=['PATCH'])
def update_simulation_profile(simulation_id: str):
    """
    更新模拟的单个Agent Profile（用于人工修正实体/人设信息）

    请求（JSON）：
        {
            "platform": "reddit",                // 可选: reddit|twitter，默认reddit
            "profile_id": "12",                  // 必填: profile标识
            "match_by": "user_id",              // 可选: user_id|username，默认user_id
            "updates": {                          // 必填: 待更新字段
                "bio": "...",
                "persona": "..."
            }
    ...
    try:
        ...
        platform = (data.get('platform') or 'reddit').strip().lower()
        profile_id = data.get('profile_id')
        match_by = (data.get('match_by') or 'user_id').strip()
        ...
        if match_by not in ['user_id', 'username']:
            return jsonify({
                "success": False,
                "error": "match_by must be 'user_id' or 'username'"
            }), 400
        ...
        if platform == 'reddit':
            ...
            if not os.path.exists(profile_path):
                return jsonify({
                    "success": False,
                    "error": "reddit_profiles.json not found"
                }), 404
            ...
            if not isinstance(profiles, list):
                return jsonify({
                    "success": False,
                    "error": "invalid reddit profiles format"
                }), 500

            target_idx = -1
            target_value = str(profile_id)
            for idx, profile in enumerate(profiles):
                current_value = profile.get(match_by)
            ...
            if target_idx < 0:
                return jsonify({
                    "success": False,
                    "error": f"profile not found by {match_by}={profile_id}"
                }), 404

            editable_fields = {
                'name', 'username', 'bio', 'persona', 'karma', 'age',
            ...
            if not applied_fields:
                return jsonify({
                    "success": False,
                    "error": "no editable fields found in updates"
                }), 400
        ...
        # twitter CSV profiles
        profile_path = os.path.join(sim_dir, 'twitter_profiles.csv')
        if not os.path.exists(profile_path):
            return jsonify({
                "success": False,
                "error": "twitter_profiles.csv not found"
            }), 404
        ...
        for idx, row in enumerate(rows):
            current_value = row.get(match_by)
            if str(current_value) == target_value:
                target_idx = idx
                break
        ...
        editable_fields = {'name', 'username', 'user_char', 'description'}
        applied_fields = []
        ...
        # Keep header stable and include any new editable fields if file is missing them.
        ...
        logger.error(f"更新Profile失败: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
